...
let previousImage = profileImg.src; // 이전이미지 기록 (초기 상태의 이미지 URL 저장)
    ...
    if(file) { // 파일이 선택된 경우
        if(file.size <= MAX_SIZE) { // 파일 크기가 허용범위 이내인 경우
            ...
            if(prevoiusFile) {
                ...
                // DataTransfer.items.remove() : 파일 삭제
                // DataTransfer.files : FileList 객체를 반환
                // -> <input type="file"> 요소에 파일을 동적으로 설정 가능
                // --> input 태그의 files 속성은 FIleList만 저장 가능하기 떄문에
